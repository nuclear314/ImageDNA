...
# %APPDATA%\ImageDNA. Under Flatpak this resolves to the sandbox-redirected
# ~/.var/app/<app-id>/data automatically — no Flatpak-specific handling
# needed here, XDG_DATA_HOME is exactly the variable Flatpak overrides.
...
# Unlike linux/main.py (a PyInstaller-frozen launcher with its own separately
# embedded python-build-standalone server runtime under sys.executable's
# sibling server/ dir), the Flatpak build installs server.py and this
# launcher into the same app directory (/app/lib/imagedna/, see the manifest)
# and runs both under the *same* Flatpak-provided Python — flatpak-pip-generator
# installs every dependency (Flask/onnxruntime/numpy/... AND pywebview/
# PyGObject) into one environment, so there's no separate interpreter to
# locate. sys.executable is that one interpreter.
...
if not os.path.exists(server_py):
    _fatal(
        ...
        f'Could not find server.py.\n\nExpected:\n{server_py}\n\n'
        f'The Flatpak app module may not have installed correctly.')
    sys.exit(1)
...
# Start Flask under the same Flatpak-provided Python; capture output to log file
...
try:
    ...
    # backend auto-detection, so the sandbox's own GTK/WebKit2GTK (from the
    # GNOME runtime, matched/tested rather than build-machine-frozen — see
    # the flatpak/ manifest's comment on why this replaced the AppImage
    # pipeline) is always what gets used.
